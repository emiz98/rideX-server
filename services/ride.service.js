...
exports.createRide = async (rider, rider_id) => {
    ...
    const isRideExits = await Ride.exists({ assigned_rider: rider.assigned_rider });
    const isClientExits = await Client.exists({ request_partner: rider.assigned_rider });
...
exports.updatePartners = async (partner, email, id) => {
    ...
    const UpdatedClient = await Client.findOneAndUpdate(clientQuery, updateClient);
...
exports.deleteRide = async (riderId) => {
    ...
    const clientResponse = await Client.updateMany(query, { $unset: { "assigned_rider": "" } });
...
exports.deletePartner = async (riderId, partnerId) => {
    ...
    const clientResponse = await Client.updateOne(query, { $unset: { "assigned_rider": "" } });
...
exports.getNearby = async (rider_email) => {
    ...
    if (ride.length === 0 || clientRequests.length === 0) {
        ...
        const nearbyUsers = getNearbyUsers(radius, ride.assigned_rider.location, clientRequests);
        return nearbyUsers
    }
